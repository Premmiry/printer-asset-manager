    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="max-w-6xl xl:max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        ...
        <div className="hidden md:grid grid-cols-2 gap-6 md:gap-8">
            ...
            <section className="bg-white rounded-3xl p-5 sm:p-6 shadow-sm border border-slate-100">
              ...
              <div className="space-y-3">
                {companies.map((comp) => (
                  <div key={comp.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl group">
                    <div className="flex items-center">
                      <span className="text-xs font-bold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded mr-2">{comp.code}</span>
                      <span className={`font-medium mr-3 ${comp.isActive !== false ? 'text-slate-700' : 'text-slate-400 line-through'}`}>{comp.name}</span>
                      {comp.isActive !== false ? (
                        <span className="text-[10px] font-bold bg-emerald-100 text-emerald-600 px-2 py-0.5 rounded-full flex items-center gap-1"><Check size={10} /> เปิดใช้งาน</span>
                      ) : (
                        <span className="text-[10px] font-bold bg-rose-100 text-rose-600 px-2 py-0.5 rounded-full flex items-center gap-1"><X size={10} /> ปิดใช้งาน</span>
                      )}
                    </div>
                    <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-all">
                      <button onClick={() => handleToggleCompanyStatus(comp.id, comp.isActive)} className={`text-xs px-3 py-1.5 rounded-lg font-medium transition-colors ${comp.isActive !== false ? 'bg-rose-50 text-rose-600 hover:bg-rose-100' : 'bg-emerald-50 text-emerald-600 hover:bg-emerald-100'}`}>{comp.isActive !== false ? 'ปิดการแสดงผล' : 'เปิดการแสดงผล'}</button>
                      <button onClick={async () => { if (window.confirm('ยืนยันการลบบริษัทนี้?')) { await deleteDoc(doc(db, 'companies', comp.id)); } }} className="p-2 text-rose-500 hover:bg-rose-100 rounded-lg transition-colors" title="ลบบริษัท"><Trash2 size={18} /></button>
                    </div>
                  </div>
                ))}
                {companies.length === 0 && <p className="text-center py-8 text-slate-400 text-sm italic">ยังไม่มีข้อมูลบริษัท</p>}
              </div>
            </section>
          )}
              ...
              {filteredDepartments.map((dept) => (
                <div key={dept.id} className="flex items-center justify-between p-3 sm:p-4 bg-slate-50 hover:bg-slate-100 rounded-2xl group transition-colors border border-slate-100/50">
                  <div className="flex items-center gap-3">
                    <div className="flex flex-col gap-1">
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-bold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded border border-indigo-100">{dept.companyCode}</span>
                        <span className="text-xs font-bold text-slate-500 bg-white px-2 py-0.5 rounded shadow-sm border border-slate-200">{dept.code}</span>
                      </div>
                      {editingDeptId === dept.id ? (
                        <div className="flex items-center gap-2">
                          <input type="text" value={editingDeptName} onChange={(e) => setEditingDeptName(e.target.value)} className="px-3 py-1.5 rounded-lg border border-slate-200 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" />
                          <button onClick={() => handleUpdateDepartmentName(dept.id)} className="p-1.5 bg-emerald-50 text-emerald-600 rounded-lg hover:bg-emerald-100" title="บันทึกชื่อแผนก"><Check size={16} /></button>
                          <button onClick={() => { setEditingDeptId(null); setEditingDeptName(''); }} className="p-1.5 bg-slate-100 text-slate-600 rounded-lg hover:bg-slate-200" title="ยกเลิก"><X size={16} /></button>
                        </div>
                      ) : (
                        <span className="font-semibold text-slate-700 text-sm">{dept.thaiName}</span>
                      )}
                  ...
                  <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-all">
                    {editingDeptId === dept.id ? null : (
                      <button onClick={() => { setEditingDeptId(dept.id); setEditingDeptName(dept.thaiName); }} className="p-2 text-indigo-600 hover:bg-indigo-100 rounded-xl" title="แก้ไขชื่อแผนก"><Pencil size={16} /></button>
                    )}
                    <button onClick={() => handleDelete(dept.id)} className="p-2 text-rose-500 hover:bg-rose-100 hover:text-rose-600 rounded-xl transition-all shadow-sm" title="ลบแผนก"><Trash2 size={18} /></button>
                  </div>
                </div>
              ))}
              {filteredDepartments.length === 0 && !uploading && (
            ...
            <div className="col-span-2 grid grid-cols-2 gap-6 md:gap-8">
              <section className="bg-white rounded-3xl p-5 sm:p-6 shadow-sm border border-slate-100">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
                  ...
                  {printerTypes.length === 0 && (
                    <button onClick={async () => { const defaults = ['Laser', 'Inkjet', 'Dot Matrix', 'Thermal', 'Multifunction']; setLoading(true); try { await Promise.all(defaults.map(name => addDoc(collection(db, 'printerTypes'), { name, createdAt: Date.now() }))); } finally { setLoading(false); } }} className="text-xs bg-indigo-50 text-indigo-600 px-3 py-1.5 rounded-lg hover:bg-indigo-100 transition-colors font-medium">โหลดข้อมูลเริ่มต้น</button>
                  )}
                </div>
                <form onSubmit={handleAddPrinterType} className="flex flex-col sm:flex-row gap-2 mb-6">
                  <input required type="text" placeholder="เพิ่มประเภทใหม่" value={newPrinterType} onChange={(e) => setNewPrinterType(e.target.value)} className="flex-1 px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500 outline-none transition-all" />
                  <button disabled={loading} type="submit" className="p-3 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 transition-colors disabled:opacity-50"><Plus size={24} /></button>
                </form>
                <div className="space-y-3 max-h-80 overflow-y-auto pr-2 custom-scrollbar">
                  {printerTypes.map((type) => (
                    <div key={type.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-2xl group">
                      {editingTypeId === type.id ? (
                        <div className="flex items-center gap-2 flex-1">
                          <input type="text" value={editingTypeName} onChange={(e) => setEditingTypeName(e.target.value)} className="flex-1 px-3 py-1.5 rounded-lg border border-slate-200 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" />
                          <button onClick={() => handleUpdatePrinterTypeName(type.id)} className="p-1.5 bg-emerald-50 text-emerald-600 rounded-lg hover:bg-emerald-100"><Check size={16} /></button>
                          <button onClick={() => { setEditingTypeId(null); setEditingTypeName(''); }} className="p-1.5 bg-slate-100 text-slate-600 rounded-lg hover:bg-slate-200"><X size={16} /></button>
                        </div>
                      ) : (
                        <>
                          ...
                          <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100">
                            <button onClick={() => { setEditingTypeId(type.id); setEditingTypeName(type.name); }} className="p-2 text-indigo-600 hover:bg-indigo-100 rounded-lg" title="แก้ไขชื่อประเภท"><Pencil size={16} /></button>
                            <button onClick={async () => { if (window.confirm(`ยืนยันการลบประเภท "${type.name}"?`)) { await deleteDoc(doc(db, 'printerTypes', type.id)); } }} className="p-2 text-rose-500 hover:bg-rose-100 rounded-lg transition-colors" title="ลบประเภท"><Trash2 size={16} /></button>
                          </div>
                        </>
                      )}
                    </div>
                  ))}
                  {printerTypes.length === 0 && <p className="text-center py-4 text-slate-400 text-sm italic">ยังไม่มีข้อมูลประเภท</p>}
                </div>
              </section>
              ...
              <section className="bg-white rounded-3xl p-5 sm:p-6 shadow-sm border border-slate-100">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
                  ...
                  {printerBrands.length === 0 && (
                    <button onClick={async () => { const defaults = ['Epson', 'Canon', 'Brother', 'HP', 'Samsung', 'Pantum', 'Ricoh', 'Oki', 'Toshiba', 'Label / Barcode Printer']; setLoading(true); try { await Promise.all(defaults.map(name => addDoc(collection(db, 'printerBrands'), { name, createdAt: Date.now() }))); } finally { setLoading(false); } }} className="text-xs bg-indigo-50 text-indigo-600 px-3 py-1.5 rounded-lg hover:bg-indigo-100 transition-colors font-medium">โหลดข้อมูลเริ่มต้น</button>
                  )}
                </div>
                <form onSubmit={handleAddPrinterBrand} className="flex flex-col sm:flex-row gap-2 mb-6">
                  <input required type="text" placeholder="เพิ่มยี่ห้อใหม่" value={newPrinterBrand} onChange={(e) => setNewPrinterBrand(e.target.value)} className="flex-1 px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500 outline-none transition-all" />
                  <button disabled={loading} type="submit" className="p-3 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 transition-colors disabled:opacity-50"><Plus size={24} /></button>
                </form>
                <div className="space-y-3 max-h-80 overflow-y-auto pr-2 custom-scrollbar">
                  {printerBrands.map((brand) => (
                    <div key={brand.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-2xl group">
                      {editingBrandId === brand.id ? (
                        <div className="flex items-center gap-2 flex-1">
                          <input type="text" value={editingBrandName} onChange={(e) => setEditingBrandName(e.target.value)} className="flex-1 px-3 py-1.5 rounded-lg border border-slate-200 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" />
                          <button onClick={() => handleUpdatePrinterBrandName(brand.id)} className="p-1.5 bg-emerald-50 text-emerald-600 rounded-lg hover:bg-emerald-100"><Check size={16} /></button>
                          <button onClick={() => { setEditingBrandId(null); setEditingBrandName(''); }} className="p-1.5 bg-slate-100 text-slate-600 rounded-lg hover:bg-slate-200"><X size={16} /></button>
                        </div>
                      ) : (
                        <>
                          ...
                          <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100">
                            <button onClick={() => { setEditingBrandId(brand.id); setEditingBrandName(brand.name); }} className="p-2 text-indigo-600 hover:bg-indigo-100 rounded-lg" title="แก้ไขชื่อยี่ห้อ"><Pencil size={16} /></button>
                            <button onClick={async () => { if (window.confirm(`ยืนยันการลบยี่ห้อ "${brand.name}"?`)) { await deleteDoc(doc(db, 'printerBrands', brand.id)); } }} className="p-2 text-rose-500 hover:bg-rose-100 rounded-lg transition-colors" title="ลบยี่ห้อ"><Trash2 size={16} /></button>
                          </div>
                        </>
                      )}
                    </div>
                  ))}
                  {printerBrands.length === 0 && <p className="text-center py-4 text-slate-400 text-sm italic">ยังไม่มีข้อมูลยี่ห้อ</p>}
                </div>
              </section>
            </div>
          )}
          ...
          {isAdmin && (
            <section className="col-span-2 bg-white rounded-3xl p-5 sm:p-6 shadow-sm border border-slate-100">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-lg font-bold text-slate-900 flex items-center gap-2">
                  <Users size={20} className="text-indigo-600" />
                  จัดการผู้ใช้งาน (Users)
              ...
              <div className="space-y-3">
                {users.map((u) => (
                  <div key={u.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl group">
                    <div>
                      <span className="font-medium text-slate-700 mr-2">{u.username}</span>
                      <span className={`text-xs font-bold px-2 py-0.5 rounded mr-2 ${u.role === 'admin' ? 'bg-rose-100 text-rose-600' : 'bg-emerald-100 text-emerald-600'}`}>{u.role}</span>
                      {u.role !== 'admin' && (<span className="text-xs font-bold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded">{u.companyCode}</span>)}
                    </div>
                    {u.uid !== userProfile?.uid && u.username.toLowerCase() !== 'prem' && (
                      <button onClick={() => handleToggleUserRole(u.uid, u.role)} className={`text-xs px-3 py-1.5 rounded-lg font-medium transition-colors opacity-0 group-hover:opacity-100 ${u.role === 'admin' ? 'bg-rose-50 text-rose-600 hover:bg-rose-100' : 'bg-emerald-50 text-emerald-600 hover:bg-emerald-100'}`}>
                        {u.role === 'admin' ? 'ลดสิทธิ์เป็น User' : 'เลื่อนเป็น Admin'}
                      </button>
                    )}
                  </div>
                ))}
                {users.length === 0 && (<p className="text-center py-8 text-slate-400 text-sm italic">ยังไม่มีข้อมูลผู้ใช้งาน</p>)}
              </div>
            </section>
          )}
        </div>
